...
class RocketAnn(BasePipe):
    def __init__(self, test_mode: bool = False) -> None:
        ...
        self._feature_extractor: Optional[Rocket] = None
    ...
    def _prepare_input(self, x: Data, training: bool = True) -> np.ndarray:
        ...
        if isinstance(x.get_temporal(), pd.DataFrame):
            ...
            x.set_temporal(x_temp)

            LOG.debug("Apply Rocket")
            x_temp, _ = x.get_temporal_3d()
            LOG.info("Rocket Before Transpose Shape: %s", x_temp.shape)
            x_temp = x_temp.transpose(0, 2, 1)
            LOG.info("Rocket Before Shape: %s", x_temp.shape)
            if training:
                self._feature_extractor: Rocket = Rocket(num_kernels=self.n_kernels, normalise=False)
                self._feature_extractor.fit(x_temp)
            x_temp = self._feature_extractor.transform(x_temp)
            LOG.info("Rocket After Shape: %s", x_temp.shape)
