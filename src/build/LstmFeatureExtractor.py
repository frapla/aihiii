...
class LSTMUniversal(BasePipe):
    def __init__(self, test_mode: bool = False) -> None:
        ...
        self.kernel_regularizer: Optional[Literal["l1", "l2"]] = None
        self.lstm_units: List[List[int]] = [[100]]  # outer list for lstm width, inner list for stack, value for units
    ...
    def _prepare_input(self, x: Data, training: bool = True) -> List[np.ndarray]:
        ...
        if isinstance(x.get_temporal(), pd.DataFrame):
            ...
            if not self.share_lstm:
                x_in_ = np.split(x_in_, x_in_.shape[2], axis=2)
            x_in.append(x_in_)
    ...
    def _build(self, x: Data, y: Data) -> None:
        ...
        if isinstance(x.get_temporal(), pd.DataFrame):
            # input
            temp_in = self._temporal_input(x=x.get_temporal())

            # feature extractor
            feature_extractor = self._set_feature_extractor(temp_in=temp_in)
            feature_extractor_block = feature_extractor
        else:
    ...
    def _set_feature_extractor(self, temp_in: Union[Input, List[Input]]) -> None:
        LOG.debug("Assemble feature extractor")
        if self.share_lstm:
            feature_extractor_in = temp_in
        else:
            feature_extractor_in = []
            for inp in temp_in:
                feature_extractor_in.append(inp)
        LOG.debug("Add Feature Extractor Input Layer: %s", feature_extractor_in)

        if self.share_lstm:
            temp_blocks = []
            # self.lstm_units outer list for lstm width, inner list for stack, value for units
            for lstm_stacks in self.lstm_units:
                temp_block = None
                for i, lstm_units in enumerate(lstm_stacks, 1):
                    if self.dropout_rate:
                        temp_block = Dropout(rate=self.dropout_rate)(feature_extractor_in if temp_block is None else temp_block)
                        LOG.debug("Add Dropout Layer %s", temp_block)
                    if self.is_bidirectional:
                        temp_block = Bidirectional(
                            LSTM(
                                units=lstm_units,
                                return_sequences=self.use_sequence_output if i == len(lstm_stacks) else True,
                                kernel_regularizer=self.kernel_regularizer,
                            )
                        )(feature_extractor_in if temp_block is None else temp_block)
                        LOG.debug("Add Bidirectional LSTM Layer %s", temp_block)
                    else:
                        temp_block = LSTM(
                            units=lstm_units,
                            return_sequences=self.use_sequence_output if i == len(lstm_stacks) else True,
                        ...
                        LOG.debug("Add LSTM Layer %s", temp_block)
                    if self.dropout_rate:
                        temp_block = Dropout(rate=self.dropout_rate)(temp_block)
                        LOG.debug("Add Dropout Layer %s", temp_block)
                temp_block = Flatten(data_format=self._channel_format)(temp_block)
                LOG.debug("Add Flatten Layer %s", temp_block)
                temp_blocks.append(temp_block)

            temp_block_ = Concatenate()(temp_blocks) if len(temp_blocks) > 1 else temp_blocks[0]

            self._feature_extractor: keras.Model = keras.Model(
                inputs=temp_blocks, outputs=temp_block_, name=self._str.feature_extractor
            )
        else:
            temp_blocks_ = []
            for inp in feature_extractor_in:
                temp_blocks = []
                for lstm_stacks in self.lstm_units:
                    temp_block = None
                    for i, lstm_units in enumerate(lstm_stacks, 1):
                        if self.dropout_rate:
                            temp_block = Dropout(rate=self.dropout_rate)(inp if temp_block is None else temp_block)
                            LOG.debug("Add Dropout Layer %s", temp_block)
                        if self.is_bidirectional:
                            temp_block = Bidirectional(
                                LSTM(
                            ...
                            )(inp if temp_block is None else temp_block)
                            LOG.debug("Add Bidirectional LSTM Layer %s", temp_block)
                        else:
                            temp_block = LSTM(
                                units=lstm_units,
                        ...
                        LOG.debug("Add Flatten Layer %s", temp_block)
                        temp_blocks.append(temp_block)
                temp_block = Concatenate()(temp_blocks) if len(temp_blocks) > 1 else temp_blocks[0]
                temp_blocks_.append(temp_block)

            temp_block_ = Concatenate()(temp_blocks_) if len(temp_blocks_) > 1 else temp_blocks_[0]

            self._feature_extractor: keras.Model = keras.Model(
                inputs=temp_blocks_, outputs=temp_block_, name=self._str.feature_extractor
            )
    ...
    def _temporal_input(self, x: pd.DataFrame) -> Union[Layer, List[Layer]]:
        """temporal input
        ...
        if self.share_lstm:
            ...
            for col in x.columns:
                inp.append(Input(shape=(n_tsps, 1), name=f"TemporalIn_{col}"))
                LOG.debug("Add Temporal Input Layer %s: %s", col, inp[-1])
        return inp
    ...
    def _dense(self) -> Union[List[Layer], List[List[Layer]]]:
        ...
        if self.share_dense:
            ...
            for dense_layer_shape in self.dense_layer_shapes:
                dense.append(Dense(dense_layer_shape, activation="relu", kernel_regularizer=self.kernel_regularizer))
            ...
            for _ in self._label_names:
                dense_vert = []
                for dense_layer_shape in self.dense_layer_shapes:
                    dense_vert.append(Dense(dense_layer_shape, activation="relu", kernel_regularizer=self.kernel_regularizer))
...
if __name__ == "__main__":
    ...
    checker = CheckPipe(pipe=LSTMUniversal, n_samples=5)
    if False:
        ...
        m = LSTMUniversal(test_mode=True)
